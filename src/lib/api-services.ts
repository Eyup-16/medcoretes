...
export class StudentService {
  ...
  static async getTodos(params: PaginationParams & {
    ...
    return apiClient.get<PaginatedResponse<any>>(`/students/todos?${searchParams.toString()}`);
  }
  ...
  static async createTodo(todoData: {
    ...
    const apiData: any = {
      ...
      type: todoData.type || 'READING'
    };

    // Only include dueDate if it's provided and not empty
    if (todoData.dueDate && todoData.dueDate.trim()) {
      apiData.dueDate = todoData.dueDate;
    }

    return apiClient.post<any>('/students/todos', apiData);
  }

  /**
   * Update a todo
   */
  static async updateTodo(todoId: number, todoData: {
    title?: string;
    description?: string;
    dueDate?: string;
    priority?: 'LOW' | 'MEDIUM' | 'HIGH';
    ...
    return apiClient.put<any>(`/students/todos/${todoId}`, apiData);
  }

  /**
   * Mark todo as complete
   */
  static async completeTodo(todoId: number): Promise<ApiResponse<any>> {
    return apiClient.put<any>(`/students/todos/${todoId}/complete`);
  }
...
export class ContentService {
  ...
  static async getCourseResources(courseId: number, params: {
    ...
    return apiClient.get<PaginatedResponse<any>>(`/courses/${courseId}/resources?${queryParams.toString()}`);
  }
...
export class QuizService {
  ...
  static async submitAnswersBulk(sessionId: number, answers: Array<{
    ...
    const requestBody = {
      answers: answers.map(answer => ({
        ...
        ...(answer.timeSpent && { timeSpent: answer.timeSpent })
      }))
    };

    return apiClient.post<any>(`/quiz-sessions/${sessionId}/submit-answer`, requestBody);
  }
  ...
  static async updateAnswer(sessionId: number, questionId: number, answerData: {
    ...
    return apiClient.put<any>(`/quiz-sessions/${sessionId}/questions/${questionId}/answer`, answerData);
  }
  ...
  static async completeQuizSession(sessionId: number): Promise<ApiResponse<any>> {
    return apiClient.post<any>(`/quiz-sessions/${sessionId}/complete`);
  }
  ...
  static async getQuizSessions(params: PaginationParams & {
    ...
    return apiClient.get<PaginatedResponse<any>>(`/quiz-sessions?${searchParams.toString()}`);
  }
}
...
export class ExamService {
  ...
  static async createExamSession(examId: number): Promise<ApiResponse<ExamSession>> {
    return apiClient.post<ExamSession>('/exams/exam-sessions', { examId });
  }
  ...
  static async submitExamAnswer(sessionId: number, answer: {
    ...
    // Use unified quiz session answer submission
    return QuizService.submitAnswer(sessionId, {
      questionId: answer.questionId,
      selectedAnswerId: answer.selectedAnswerId || 0,
