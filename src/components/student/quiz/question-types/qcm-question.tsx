...
export function QCMQuestion({ question }: Props) {
  ...
  const handleOptionToggle = (optionId: string) => {
    // Prevent interaction if session is completed
    if (session.status === 'completed' || session.status === 'COMPLETED') return;
    if (isAnswerRevealed && hasSubmitted) return;
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-foreground font-medium">
                Sélectionnez toutes les réponses correctes ({correctCount} réponse{correctCount > 1 ? 's' : ''} attendue{correctCount > 1 ? 's' : ''})
              </p>
            </div>
      ...
      <div className="space-y-4">
        {question.options.map((option, index) => {
          const status = getOptionStatus(option);
          const isSelected = selectedOptions.includes(option.id);
          const isDisabled = isAnswerRevealed && hasSubmitted;
        ...
        <div className="flex gap-2">
          ...
          {!hasSubmitted && (
            <Button
              ...
              disabled={selectedOptions.length === 0 || session.status === 'completed' || session.status === 'COMPLETED'}
              size="sm"
            >
        ...
        <Card className="bg-muted/50">
          <CardContent className="pt-4">
            <div className="space-y-2">
              <h4 className="text-sm font-medium">Résumé de votre réponse :</h4>
              <div className="grid grid-cols-2 gap-4 text-xs">
                ...
                <div>
                  <span className="text-muted-foreground">Vos réponses :</span>
                  <div className="mt-1">
                    ...
                    {selectedOptions.length === 0 && (
                      <div className="text-muted-foreground">Aucune réponse</div>
                    )}
                  </div>
                </div>
              </div>
