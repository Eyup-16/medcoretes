...
export function QuizLayout() {
  ...
  const { state, pauseQuiz, resumeQuiz, nextQuestion, previousQuestion, completeQuiz, goToQuestion, toggleSidebar } = useQuiz();
  ...
  // Show completion screen when all questions are answered (but not yet submitted)
  ...
  // Show completion screen when user has answered all questions
  if (allQuestionsAnswered && session.status !== 'completed') {
    return <QuizCompletionScreen />;
  }

  // Show results if session is completed (after submission)
  if (session.status === 'completed') {
    // Check if we're using API context by looking for API-specific state
    const isApiContext = 'apiSessionId' in state;
    return isApiContext ? <ApiQuizResults /> : <QuizResults />;
  }
      ...
      <header className="border-b bg-card/50 backdrop-blur-sm supports-[backdrop-filter]:bg-card/50 shadow-sm flex-shrink-0">
        <div className="flex items-center justify-between px-3 sm:px-4 lg:px-6 py-2 sm:py-3">
          ...
          <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
            {/* Client Storage Status - Hidden on mobile */}
            <div className="hidden lg:block">
              <ClientStorageIndicator />
            </div>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0 relative">
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-gradient-to-br from-background via-background to-muted/10 min-w-0">
          ...
          <EnhancedQuizFooter
            ...
            onNext={nextQuestion}
            onSubmit={() => {
              // Force show completion screen by navigating to the end
              if (currentQuestionIndex < totalQuestions - 1) {
                // Go to last question first, then completion screen will show
                for (let i = currentQuestionIndex; i < totalQuestions - 1; i++) {
                  nextQuestion();
                }
              }
            }}
