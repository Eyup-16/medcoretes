...
export function ClientStorageIndicator() {
  ...
  const getStatusInfo = () => {
    ...
    if (answersCount > 0) {
      ...
      return {
        icon: Save,
        color: allAnswered ? 'text-blue-500' : 'text-green-500',
        bgColor: allAnswered ? 'bg-blue-50 dark:bg-blue-950/20' : 'bg-green-50 dark:bg-green-950/20',
        borderColor: allAnswered ? 'border-blue-200 dark:border-blue-800' : 'border-green-200 dark:border-green-800',
...
export function ClientStorageIndicatorCompact() {
  ...
  const getStatusColor = () => {
    if (state.pendingSubmission) return 'text-blue-500';
    if (state.session.status === 'COMPLETED' && answersCount > 0) return 'text-orange-500';
    if (answersCount > 0) return 'text-green-500';
