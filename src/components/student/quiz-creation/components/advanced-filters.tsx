...
export function AdvancedFilters({
  ...
  const { filteredCount, reductions } = useFilterImpact(
    baseQuestionCount,
    filters,
    { quizSources: availableSources, availableQuizYears: availableYears } as any
  );
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              Select specific exam types and sources (affects question variety)
            </p>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {availableSources.map((source, index) => {
                const isSelected = filters.quizSourceIds?.includes(source.id) || false;
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => handleSourceToggle(source.id, !isSelected)}
                  >
                    <Checkbox
                      checked={isSelected}
                    ...
                    >
                      <div className="font-medium text-sm leading-tight">{source.name}</div>
                      {source.description && (
            ...
            {filters.quizSourceIds?.length === 0 && (
              <motion.p
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                No sources selected - all sources will be included
              </motion.p>
            )}
          </CardContent>
        </Card>
      ...
      >
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              Select specific years for questions (affects question currency)
            </p>
          </CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-3">
              {availableYears.map((year, index) => {
                const isSelected = filters.quizYears?.includes(year) || false;
      ...
      >
        <Card className="border-blue-200/60 bg-blue-50/30 hover:bg-blue-50/40 hover:border-blue-200/80">
