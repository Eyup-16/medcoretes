...
export function useQuestionCalculation(
  ...
  return useMemo(() => {
    ...
    // Only apply reductions if user has specifically selected some sources/years
    if (filters.quizSourceIds?.length && filterData.quizSources && filters.quizSourceIds.length < filterData.quizSources.length) {
      const sourceReduction = 1 - (filters.quizSourceIds.length / filterData.quizSources.length);
      const reduction = Math.floor(totalQuestions * sourceReduction * 0.3);
      totalQuestions -= reduction;
    }

    if (filters.quizYears?.length && filterData.availableQuizYears && filters.quizYears.length < filterData.availableQuizYears.length) {
      const yearReduction = 1 - (filters.quizYears.length / filterData.availableQuizYears.length);
...
export function useFilterImpact(
  ...
  return useMemo(() => {
    ...
    if (advancedFilters.quizSourceIds?.length && filterData.quizSources) {
      const totalSources = filterData.quizSources.length;
    ...
    if (advancedFilters.quizYears?.length && filterData.availableQuizYears) {
      const totalYears = filterData.availableQuizYears.length;
