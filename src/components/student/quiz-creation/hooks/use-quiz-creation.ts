...
export function useQuizCreation(
  ...
  // External hooks
  const { filters: filterData, loading: filtersLoading } = useQuizFilters();
    ...
    try {
      ...
      const quizPayload = {
        ...
        filters: {
          ...
          courseIds: config.filters.courseIds,
          // Only include these fields if they have actual selections (not empty arrays)
