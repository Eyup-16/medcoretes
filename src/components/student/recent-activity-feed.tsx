...
export function RecentActivityFeed({ activities }: RecentActivityFeedProps) {
  ...
  const combinedActivities = React.useMemo(() => {
    ...
    // Convert recent sessions to activity format
  ...
  if ((!combinedActivities || combinedActivities.length === 0) && !sessionsLoading) {
    return (
      <Card className="relative overflow-hidden bg-gradient-to-br from-background/95 via-accent/8 to-muted/12 border border-border/50 shadow-xl">
        ...
        <CardContent className="relative">
          <div className="text-center py-[calc(var(--spacing)*8)]">
            ...
            <Button className="gap-[calc(var(--spacing)*2)] bg-gradient-to-r from-chart-1 to-chart-2 hover:opacity-90 text-white shadow-lg hover:shadow-xl transition-all duration-300 font-semibold tracking-tight">
    ...
    <Card className="relative overflow-hidden bg-gradient-to-br from-background/95 via-accent/8 to-muted/12 border border-border/50 shadow-xl hover:shadow-2xl transition-all duration-300">
      ...
      <CardHeader className="relative pb-[calc(var(--spacing)*4)]">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-[calc(var(--spacing)*3)] sm:gap-[calc(var(--spacing)*4)]">
          ...
          <Button variant="outline" size="sm" className="bg-card/50 backdrop-blur-sm gap-[calc(var(--spacing)*2)] font-medium self-start sm:self-auto flex-shrink-0 border-chart-1/30 hover:border-chart-1/50 transition-all">
      ...
      <CardContent className="relative space-y-[calc(var(--spacing)*4)]">
          ...
          <div className="bg-gradient-to-r from-chart-1/12 to-chart-2/12 rounded-xl p-[calc(var(--spacing)*5)] border border-chart-1/20 hover:border-chart-1/30 transition-all duration-300 shadow-lg hover:shadow-xl">
            ...
            <div className="relative">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-[calc(var(--spacing)*3)] sm:gap-[calc(var(--spacing)*4)]">
                ...
                <Button variant="ghost" size="sm" className="h-9 px-[calc(var(--spacing)*4)] text-sm font-medium self-start sm:self-auto bg-card/50 hover:bg-card/70 transition-all">
                  Review Session
                </Button>
              </div>
            </div>
              ...
              {otherActivities.map((activity, index) => {
                ...
                return (
                  <div
                    ...
                    className="group flex items-center gap-[calc(var(--spacing)*3)] p-[calc(var(--spacing)*4)] bg-gradient-to-r from-card/80 to-card/60 backdrop-blur-sm rounded-lg border border-border/20 hover:border-border/40 transition-all duration-300 cursor-pointer hover:shadow-md"
                  >
        ...
        <div className="pt-[calc(var(--spacing)*5)] border-t border-gradient-to-r from-transparent via-border to-transparent">
          <div className="bg-gradient-to-r from-background/50 to-card/30 rounded-lg p-[calc(var(--spacing)*4)] border border-border/30">
            ...
            <Button className="w-full gap-[calc(var(--spacing)*2)] bg-gradient-to-r from-chart-1 to-chart-2 hover:opacity-90 text-white shadow-lg hover:shadow-xl transition-all duration-300 font-semibold tracking-tight">
              <Play className="h-4 w-4" />
              Start New Session
            </Button>
          </div>
