    ...
    <div className={`space-y-2 ${className}`}>
      {stages.map((stage, index) => (
        <div key={index} className="flex items-center gap-3">
          ...
          <span className={`text-sm ${stage.completed ? 'text-green-600 dark:text-green-400' : stage.loading ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>
            {stage.name}
          </span>
        </div>
      ))}
